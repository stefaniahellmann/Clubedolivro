...
import { useRaffle } from '../contexts/RaffleContext'; // para a seção Rifa
...
export function AdminPanel() {
  ...
  const menuItems = [
    ...
    { id: 'links' as AdminSection, label: 'Acessos Rápidos (Links)', icon: Settings },
    { id: 'raffle' as AdminSection, label: 'Rifa', icon: Ticket },
  ];
      ...
      >
        <div className="p-6 border-b border-zinc-200 dark:border-zinc-800">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Painel Admin</h2>
            <Button
              onClick={logout}
              variant="ghost"
              size="sm"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="flex items-center">
          ...
          <div className="ml-4">
            <h3 className="text-2xl font-bold text-zinc-900 dark:text-white">5</h3>
...
/* =========================
 *   LINKS (Acessos rápidos)
 * ========================= */

type QuickLink = {
  id: number;
  key: 'drive' | 'whats' | 'telegram' | 'invite' | 'rules';
  label: string;
  type: 'link' | 'share' | 'modal';
  url?: string;
  shareText?: string;
...
function LinkManagement() {
  ...
  const [links, setLinks] = useState<QuickLink[]>([
    { id: 1, key: 'drive', label: 'Drive de Materiais', type: 'link', url: 'https://seu-drive.com', badge: 'Acesse +1k Volumes' },
    { id: 2, key: 'whats', label: 'Clube Whats', type: 'link', url: 'https://wa.me/seu-grupo', badge: 'Grupo WhatsApp' },
    { id: 3, key: 'telegram', label: 'Clube Telegram', type: 'link', url: 'https://t.me/seu-grupo', badge: 'Grupo Telegram' },
    { id: 4, key: 'invite', label: 'Indique Amigos', type: 'share', shareText: 'Vem para o Clube do Livro! https://seusite.com', badge: 'Compartilhe o clube' },
    { id: 5, key: 'rules', label: 'Regras do Clube', type: 'modal', rulesMarkdown: '- Respeito\n- Sem spam\n- Foco em leitura', badge: 'Leia com atenção' },
  ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Acessos Rápidos (Home)</h1>
        ...
        <div className="space-y-4 p-4">
          {links.map((link, idx) => (
            <div
              key={link.id}
              className={`border-b border-zinc-200 dark:border-zinc-800 pb-4 ${
                idx === links.length - 1 ? 'border-b-0 pb-0' : ''
              }`}
            >
              <div className="flex justify-between items-start gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-semibold text-zinc-900 dark:text-white">
                      ...
                      <span className="text-xs font-normal text-zinc-500">
                        ({link.type}{link.badge ? ` – ${link.badge}` : ''})
                      </span>
                    </h3>
                  </div>

                  <div className="mt-2 text-sm text-zinc-700 dark:text-zinc-300 space-y-1">
                    {link.type === 'link' && <div><strong>URL:</strong> {link.url}</div>}
                    {link.type === 'share' && <div><strong>Texto para compartilhar:</strong> {link.shareText}</div>}
                    {link.type === 'modal' && (
                      <div className="prose prose-zinc dark:prose-invert">
                        <strong>Regras (Markdown):</strong>
                        <pre className="mt-1 whitespace-pre-wrap text-xs">{link.rulesMarkdown}</pre>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <Button variant="ghost" size="sm" onClick={() => openEdit(link)} className="text-amber-700 hover:bg-amber-50 dark:text-amber-300 dark:hover:bg-amber-500/10">
                    <Edit size={14} />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => del(link.id)} className="text-rose-700 hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-500/10">
                    <Trash2 size={14} />
                  </Button>
                </div>
              </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  value={editing.type}
                  onChange={(e) => setEditing({ ...editing, type: e.target.value as QuickLink['type'] })}
                  className="w-full px-3 py-2 rounded-lg bg-white border border-zinc-200 text-zinc-900 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-100"
                >
                  <option value="link">Link (abre URL)</option>
                  <option value="share">Compartilhar (texto)</option>
                  <option value="modal">Modal (regras)</option>
                </select>
              </div>
            ...
            <Input
              label="Badge (subtexto opcional)"
              value={editing.badge || ''}
              onChange={(v) => setEditing({ ...editing, badge: v })}
              placeholder="Ex.: Acesse +1k Volumes / Grupo WhatsApp / etc."
            />
            ...
            {editing.type === 'share' && (
              <Textarea
                label="Texto para compartilhar"
                rows={3}
                value={editing.shareText || ''}
                onChange={(v) => setEditing({ ...editing, shareText: v })}
              />
            )}

            {editing.type === 'modal' && (
              <Textarea
                label="Regras (Markdown)"
                rows={6}
                value={editing.rulesMarkdown || ''}
                onChange={(v) => setEditing({ ...editing, rulesMarkdown: v })}
              />
            )}
...
function RaffleAdmin() {
  ...
  const approved = state.reservations.filter((r) => r.status === 'approved');

  const filtered = pending.filter(
    (r) =>
      r.userName.toLowerCase().includes(query.toLowerCase()) ||
      r.numbers.join(',').includes(query)
  );

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Rifa — Administração</h1>

      <Card className="p-4">
        <h3 className="font-semibold text-zinc-900 dark:text-white mb-3">Reservas Pendentes</h3>
        ...
        <div className="flex items-center gap-3 mb-4">
          <input
            ...
            className="w-full sm:w-80 px-3 py-2 rounded-lg bg-white border border-zinc-200 text-zinc-900 placeholder-zinc-400 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-100"
          />
        </div>

        <div className="space-y-3">
          {filtered.length === 0 && <div className="text-sm text-zinc-500">Nenhuma reserva pendente.</div>}

          {filtered.map((r) => (
            <div key={r.id} className="p-3 rounded-lg border bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div className="text-sm text-zinc-800 dark:text-zinc-200">
                  <div><strong>{r.userName}</strong> — {new Date(r.createdAt).toLocaleString('pt-BR')}</div>
                  <div className="mt-1">
                    Números: <span className="font-medium">{r.numbers.join(', ')}</span>
                    <span className="ml-3">Total: <strong>R$ {r.amount.toFixed(2).replace('.', ',')}</strong></span>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button onClick={() => approveReservation(r.id)} className="bg-emerald-600 hover:bg-emerald-700">
                    Aprovar
                  </Button>
                  <Button variant="ghost" className="text-rose-700 hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-500/10" onClick={() => rejectReservation(r.id)}>
                    Reprovar
                  </Button>
                </div>
              </div>
      ...
      <Card className="p-4">
        <h3 className="font-semibold text-zinc-900 dark:text-white mb-3">Vendas Aprovadas</h3>
        {approved.length === 0 ? (
          <div className="text-sm text-zinc-500">Nenhuma venda aprovada ainda.</div>
        ) : (
          <div className="space-y-2">
            {approved.map((r) => (
              <div key={r.id} className="text-sm text-zinc-800 dark:text-zinc-200">
                <strong>{r.userName}</strong>: {r.numbers.join(', ')} — R$ {r.amount.toFixed(2).replace('.', ',')}
              </div>
            ))}
          </div>
        )}
      ...
      <Card className="p-4">
        <h3 className="font-semibold text-zinc-900 dark:text-white mb-3">Mapa de Números</h3>
        <div className="grid grid-cols-5 sm:grid-cols-10 md:grid-cols-15 lg:grid-cols-20 xl:grid-cols-25 gap-1">
          {state.numbers.map((x) => {
            const cls =
              x.status === 'sold'
                ? 'bg-rose-100 text-rose-800 border-rose-200 dark:bg-rose-500/10 dark:text-rose-300 dark:border-rose-700/40'
                : x.status === 'pending'
                ? 'bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-500/10 dark:text-amber-300 dark:border-amber-700/40'
                : 'bg-white text-zinc-700 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-200 dark:border-zinc-700';
            return (
              <div key={x.n} className={`text-xs rounded px-2 py-1 border ${cls}`} title={x.userName || ''}>
                {x.n}
              </div>
            );
          })}
